# Degiro transactions columns mapping

## Date,Time,Product,ISIN,Reference,Venue,Quantity,Price,,Local value,,Value,,Exchange rate,Transaction and/or third,,Total,,Order ID

`hv14729eb4c430850b9929b3d15b054a673`

| Order | CSV column title         | Halir key          |
| ----: | ------------------------ | ------------------ |
|     1 | Date                     | date               |
|     2 | Time                     | time               |
|     3 | Product                  | product            |
|     4 | ISIN                     | isin               |
|     5 | Reference                | referenceExchange  |
|     6 | Venue                    | venue              |
|     7 | Quantity                 | quantity           |
|     8 | Price                    | price              |
|     9 |                          | priceCurrency      |
|    10 | Local value              | localValue         |
|    11 |                          | localValueCurrency |
|    12 | Value                    | value              |
|    13 |                          | valueCurrency      |
|    14 | Exchange rate            | exchangeRate       |
|    15 | Transaction and/or third | fees               |
|    16 |                          | feesCurrency       |
|    17 | Total                    | total              |
|    18 |                          | totalCurrency      |
|    19 | Order ID                 | orderId            |

## Date,Time,Product,ISIN,Reference exchange,Venue,Quantity,Price,,Local value,,Value EUR,Exchange rate,AutoFX Fee,Transaction and/or third party fees EUR,Total EUR,Order ID,

`hv17128931846c45cbfa4d51e08b671500c`

| Order | CSV column title                        | Halir key          |
| ----: | --------------------------------------- | ------------------ |
|     1 | Date                                    | date               |
|     2 | Time                                    | time               |
|     3 | Product                                 | product            |
|     4 | ISIN                                    | isin               |
|     5 | Reference exchange                      | referenceExchange  |
|     6 | Venue                                   | venue              |
|     7 | Quantity                                | quantity           |
|     8 | Price                                   | price              |
|     9 |                                         | priceCurrency      |
|    10 | Local value                             | localValue         |
|    11 |                                         | localValueCurrency |
|    12 | Value EUR                               | value              |
|    13 | Exchange rate                           | exchangeRate       |
|    14 | AutoFX Fee                              | autoFxFee          |
|    15 | Transaction and/or third party fees EUR | fees               |
|    16 | Total EUR                               | total              |
|    17 | -                                       | _BUG\*_            |
|    18 | Order ID                                | orderId            |

\* There is a bug in current (2026-01-04) CSV export: more values then columns
